...
class WfsCompass(SourceCompass):
    ...
    def reset_noise(self, seed=None, noise=None) -> None:
        """ Reset all the WFS RNG to their original state
        """
        if noise is None:
            noise = self._config.p_wfss[0].noise

        for wfs_index, p_wfs in enumerate(self._config.p_wfss):
            if seed is None:
                self._wfs.d_wfs[wfs_index].set_noise(noise, 1234 + wfs_index)
            else:
                self._wfs.d_wfs[wfs_index].set_noise(noise, seed + wfs_index)
